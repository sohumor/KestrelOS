...
static const char *const state_name[] = {
    "stopped", "running", "waiting", "exited", "failed", "disabled"
};
...
struct unit {
    ...
    char after[NAMESZ];       /* dependency, services only */
    int  kind;
    int  respawn;
    int  enabled;
    int  state;
...
/* Order services so that every `after=` target starts first. Unknown or
 * self-referential targets are warned about and ignored; anything left
 * unplaced is part of a cycle and is failed rather than started. */
static void order_services(void)
{
    ...
    do {
        ...
        for (i = 0; i < nunits; i++) {
            ...
            if (u->after[0]) {
                d = find_unit(u->after);
                if (d == u) {
                    logf(K_LOG_WARN, "service %s: after=%s is itself, ignored",
                         u->name, u->after);
                } else if (!d || d->kind != K_SERVICE) {
                    logf(K_LOG_WARN, "service %s: after=%s is not a service",
                         u->name, u->after);
                } else if (d->order < 0) {
                    continue;               /* dependency not placed yet */
                }
            }
            u->order = placed++;
            changed = 1;
    ...
    for (i = 0; i < nunits; i++) {
        ...
        if (u->kind == K_SERVICE && u->order < 0) {
            logf(K_LOG_ERR,
                 "service %s: dependency cycle through after=%s, not started",
                 u->name, u->after);
            u->state = S_FAILED;
            u->order = placed++;
        }
...
static void poll_command(void)
{
    ...
    *strchr(buf, '\n') = '\0';
    ...
    ack[0] = '\0';
    if (n < 2)
        setstr(ack, sizeof(ack), "err usage: <start|stop|restart> <name>\n");
    else
...
static int any_running(void)
{
    ...
    for (i = 0; i < nunits; i++)
        if (units[i].state == S_RUNNING && units[i].pid > 0)
            return 1;
    return 0;
}
...
static void do_shutdown(int action)
{
    ...
    for (;;) {
        ...
        for (i = 0; i < nunits; i++) {
            u = &units[i];
            if (u->state != S_RUNNING || u->pid <= 0 || u->stopping)
                continue;
