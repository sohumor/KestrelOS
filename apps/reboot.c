/* reboot.c - restart the machine, through init when it is available.
 *
 * Writes "reboot" to /run/shutdown so init can stop the services in
 * reverse order first. If init has not taken the machine down within
 * ~3 seconds (or is not running at all) the kernel power syscall is
 * invoked directly.
 */
...
int main(int argc, char **argv)
{
    ...
    printf("Rebooting KestrelOS...\n");

    if (request("reboot") == 0) {
        for (waited = 0; waited < WAIT_MS; waited += STEP_MS)
            sleep_ms(STEP_MS);
        printf("reboot: init did not respond in %d ms, resetting now\n",
               WAIT_MS);
    } else {
        printf("reboot: init is not available, resetting now\n");
    }

    sleep_ms(100);
    syscall(SYS_POWER, K_POWER_REBOOT, 0, 0, 0);

    /* Only reached if the kernel could not reset the machine. */
